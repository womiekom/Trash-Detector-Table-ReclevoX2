...
const TrashDetector: React.FC = () => {
  ...
  const [currentTrashItems, setCurrentTrashItems] = useState<string[]>([]);
  ...
  const triggerTrashAlert = (trashItems: string[]) => {
    // Show visual alert
    setCurrentTrashItems(trashItems);
    ...
    // Hide visual alert after 5 seconds
    ...
    console.log('🚨 MULTI-MODAL TRASH ALERT TRIGGERED!');
  };
  ...
  const detectTrash = async () => {
    try {
      ...
      if (trashDetected && trashItemsDetected.length > 0) {
        ...
        setShowTrashAlert(false); // Hide alert if only normal items
  ...
  return (
    <>
      <TrashAlert isVisible={showTrashAlert} trashItems={currentTrashItems} />
